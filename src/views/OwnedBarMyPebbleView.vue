  ...
  methods: {
    loadData: async function() {
      ...
      for (var index in content.data) {
        ...
        if (data.userAddressHedera  === _this.$store.state.userAddressHedera ) {
      ...
      for (
        ...
        await _this.$store.state.ionftContract.methods
          ...
          .call({ from: _this.$store.state.userAddressHedera , gas: 6000000 })
          .then((details, error) => {
            if (details[0] === _this.$store.state.userAddressHedera ) {
